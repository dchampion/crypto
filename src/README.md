...
The top&ndash;level `src` folder represents the root of this package, and contains the sub&ndash;packages `api` and `core`. Together, these two packages implement the services of the aforementioned cryptosystems. The `tests` package, which itself is subdivided into `api` and `core` packages, contains unit tests for the modules in their corresponding `src`&ndash;level packages.
...
Better still, the Jupyter notebooks in the [doc](https://github.com/dchampion/crypto/blob/master/doc) folder of this repository simulate the use of this library in application&ndash;level simulations.
