...
# Bit length of a prime number q that is the order (or size) of large subgroup modulo
# p, where p is a (much larger) prime number that is the order of the full group modulo p.
_q_bit_len = 256
...
def generate_parameters(p_bit_len: int) -> tuple[int, int, int]:
    ...
    p_bit_len length (p_bit_len must be at least 2048 bits) that is the order of
    the full group modulo p, and g is a generator of the subgroup of order q.
    """
    ...
    # modulo p, within which the public keys exchanged between communicating parties
    # must fall.
...
def generate_keypair(q: int, p: int, g: int) -> tuple[int, int]:
    """
    Given the public parameters q, p and g (which are obtained from this module's
    generate_parameters function), returns a tuple of the form (k_prv, k_pub),
    ...
    generator of the subgroup modulo p that is of order q. The private key k_prv
    returned by this function must be kept secret; whereas the public key k_pub
...
def generate_session_key(k_pub: int, k_prv: int, p: int) -> bytes:
    """
    Given a private key k_prv known only to the caller of this function, and a
    public key k_pub supplied by another party, returns a hashed byte array suitable
    for use as a session key to be used by both parties in a symmetric cipher (e.g.,
    3DES, AES). The session key returned by this function must be kept secret.
    """
...
def validate_pub_key(k_pub: int, q: int, p: int) -> None:
    """
    Validates a public key k_pub given its corresponding public parameters q and p. Must be
    called, without raising an exception, by a party receiving the public key and parameters
    from another party. If this function raises an exception, the public key is invalid and
    the session should be halted.
    """
    assert isinstance(k_pub, int)
...
def validate_parameters(q: int, p: int, g: int) -> None:
    ...
    another party, or by a party generating the parameters using this module's generate_parameters
    function. If this function raises an exception, the parameters are invalid and the
