...
def new_curve(curve, B_iters=100):
    ...
    those supplied to this function. The parameter p is the field parameter
    (or modulus), a and b the curve coefficients, Gx and Gy the xy-coordinates
    of the base point on the curve, n the order of the base point [Gx, Gy] and
    h the cofactor of the subgroup generated by [Gx, Gy]. The equation for an
    elliptic curve is y^2 = x^3 + ax + b % p, where x and y are points on the
    curve (variable), a and b are the curve coefficients (constant), and the p
