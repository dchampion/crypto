...
# Point at infinity (TODO: Find a better representation???)
_pt_i = [0, 0]
...
def new_curve(p, a, b, Gx, Gy, n, h, B_iters=100):
    ...
    and y are the variables, a and b the coefficients, and p the modulus.
    """
...
def add(pt):
    """
    Returns the sum of the supplied point pt and the generator point _G. If
    pt[X] == _Gx, or pt == [0, 0] (the point at infinity), returns the point
    at infinity.
    """
    _validate_pt(pt)
...
def double(pt):
    """
    Returns the sum of supplied point pt and itself. If pt == [0, 0] (the point
    at infinity), returns the point at infinity.
    """
    _validate_pt(pt)
...
def generate_key():
    ...
    randomly generated positive integer in the range 1 <= d < _n (where _n is
    the order of the generator _G), and Q is the point on the curve resulting
    from d point-additions of _G. d, the private key, must be kept secret by
...
def _fast_point_at(d):
    ...
    d is a positive integer in the range 1 <= d < _n, and _n is the order of _G.
    In contrast with the function point_at, this function runs in O(log2(n))
    (logarithmic) time.
    """
...
def _point_at(d):
    ...
    Returns the point pt at d point-additions of the generator point _G, where
    d is a positive integer in the range 1 <= d < _n, and _n is the order of _G.
    In contrast with the function fast_point_at, this function runs in O(n)
    (linear) time.
    """
    assert isinstance(d, int)
    assert 0 < d <= _n
...
def _validate_curve_params(B_iters=100):
    ...
    assert _fast_point_at(_n) == _pt_i
